...
## Stress-test: what the ~91% factor *reversing* does *(added 2026-06-15)*
The PC1 share is usually read as a calm-regime fact ("the buckets move together"). The stress-test asks the asymmetric question: **what happens on the reversal**, when the common factor flips from risk-on to risk-off?
- **Diversification is ~illusory, quantified.** For *n* equally-weighted segments with average pairwise correlation ρ and equal vol, portfolio variance ÷ single-name variance = `1/n + (1−1/n)·ρ`. For the US-credit ladder (**n=5, ρ=0.88**): **0.904** — holding all five rating buckets removes only **~9.6%** of the variance, vs the **~80%** a zero-correlation 5-name basket would remove. The **effective number of independent bets ≈ 1/0.904 ≈ 1.1** — the cross-section is **~1.1 bets wearing 5 labels.** So on a reversal there is **almost no cushion**: AAA→BBB→CCC widen together.
- **Compression is the pre-stress tell.** Dispersion "compresses in calm, blows out in stress." Two of the four cross-sections — **municipal and TRACE-breadth — read *compressed* now**, while US credit reads *elevated*: a mixed regime where the highest-PC1 segments are precisely the ones whose dispersion is most compressed (priced for calm). Compressed dispersion historically **precedes** the blow-out, not follows it — the cross-section is primed, not relieved.
- **The reversal is the forcing event for the self-marked four.** The machine-checked `self_marked_value` U3 proves the four self-marked gaps (bank HTM, AI fair-value marks, private-credit NAV, insurance captives) **co-move under a common factor and cannot be netted**; a PC1 reversal in credit is exactly that common factor turning — so it prices **all four self-marked numbers at once**, with no asset-class diversification either. The ~91% credit PC1 and the U3 "no netting" result are two views of one fact.
- **Direction + magnitude, honestly.** The *level* of stress on a reversal is not forecastable (the date is unknowable, per `spec-unwind-timing`); what the math fixes is the **shape**: a one-factor reversal is undiversified across both the credit cross-section (~1.1 effective bets) and the four self-marked asset classes (U3), so the loss is **correlated, simultaneous, and un-hedgeable by holding "many different credits."** That is the stress-test's load-bearing claim — graded labeled interpretation on the reversal scenario, fact on the ρ=0.88 / PC1=91% inputs and the diversification arithmetic.
