...
- **CT internals (overlay, 2026-08-21, no target contact).** Tools for Humanity certificates name `deep-face-zoom.toolsforhumanity.com` and an `-ep-api` / `-internal` / `-canary` family, plus `orbs.toolsforhumanity.com` and `pgadmin.toolsforhumanity.com`. Worldcoin certificates name Orb `management`/`update` and `internal.eu-central-1.mpc.worldcoin.org`. This is the **device + MPC + face** plane behind the "proof of personhood" product, sitting next to the Zoom partnership — still **no documented OpenAI↔World technical integration**. Catalog: `osint/catalog/entities/world-tfh.md`.

**Caveat / honest limit:** there is **no documented technical integration** (no "Sign in to OpenAI with World ID," no shared user database, no shared engineering org). The overlap is *common ownership + common thesis + common investors*, not a wired-together product. OpenAI is reportedly building a "real-humans-only" social product, but no formal OpenAI↔World tie is confirmed. [Slashdot/SF Standard](https://news.slashdot.org/story/26/04/02/0345206/group-pushing-age-verification-requirements-for-ai-sneakily-backed-by-openai)
...
## 8. The forcing event — what drags the real names into daylight *(added 2026-06-15)*
The project's signature motif is the **forcing event**: a self-marked or deniable thing holds until reality prices it. Applied here — the digital-ID push is wrapped in *"child protection / anti-fraud,"* and that wrapper holds until a failure forces into daylight both **what the system actually does** (de-anonymize everyone) and **who profits**. The candidate forcing modes:
1. **National-wallet breach (not a vendor).** A mass compromise of an **EUDI national wallet or GOV.UK One Login** that *cannot* be deflected to "a third party" — forcing the **mandating government (named ministers)** to own a breach of the most sensitive data a citizen holds. The 5CA/Discord and Europa breaches (§7) are the rehearsal; a national-wallet breach is the event.
2. **Child-harm inversion.** A documented harm to a minor **caused by** the verification system (a leaked child's ID; a predator exploiting the honeypot) **inverts** the child-protection wrapper — the single event most able to flip the politics, because the stated justification becomes the demonstrated harm.
3. **Court invalidation.** A ruling (the *Van Loon* analogue) that a mandate is unlawful — naming the architecture and its sponsors and stripping the "neutral safety measure" framing.
4. **Whistleblower / code disclosure.** A **Persona-style** disclosure (Feb 2026) proving an "age check" is a **financial-intelligence + watchlist + biometric-retention** pipeline — which also names the backers (Founders Fund/Thiel), collapsing the "just checking age" deniability.
5. **Convergence becomes undeniable.** When the **same names** recur as both the **AI-creator-of-the-problem** and the **identity-seller-of-the-cure** (Altman: OpenAI + World; Blair/Ellison/Oracle + UK digital ID; the Google-Apple OS duopoly as attestation gatekeeper), the "separate, coincidental actors" deniability fails on its face. Here the forcing event is just **the map drawn plainly** — no breach required.

**The asymmetry this exposes:** the user's real name is surrendered at enrollment and de-anonymized **by design, immediately** (§7, [[digitalid-device-ownership-erosion]]); the operators stay deniable behind "child protection" **until** one of the events above. The system makes ordinary people legible while keeping its architects illegible — and only a failure reverses that. *(Scenario analysis — labeled interpretation — anchored on the documented breach record (§7) and the Van Loon precedent.)*
