...
## 7. Limits & ingest roadmap
This is an **attribution + source-pointer** layer, not a raw address dump. Government attributions are high-grade; clustering is labeled. The canonical machine-readable source is the **OFAC SDN "Digital Currency Address" set** plus FBI PSA IOC lists; a **`fetch_ofac.py`** (env-free) is the wired roadmap to ingest those into `data/ofac_crypto_addresses.json` for live cross-referencing against graph entities — parallel to `fetch_fec` / `fetch_tape`. Sanction status is dated (Tornado Cash 2022 → vacated 2024 → lifted 2025).
