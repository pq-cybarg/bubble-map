...
**Passive CT (2026-08-21, crt.name):** the stack is not just a token and a landing page. `worldcoin.org` names an Orb **management/update/auth** plane, **MPC** at `internal.eu-central-1.mpc.worldcoin.org` (the iris-code ceremony), and `vpn.consumer.worldcoin.org`. `world.org` names `login.enterprise.world.org`. Tools for Humanity names a **`deep-face-zoom*`** API family (including `-ep-api-internal`) plus **pgAdmin** on the same apex — a biometric/face plane sitting next to the Apr 2026 Zoom partnership. Named-developer sandboxes exist in CT and are **not** transcribed here. Catalog: `osint/catalog/entities/world-tfh.md`. *Fact as certificate issuance; not a claim the hosts are live.*
