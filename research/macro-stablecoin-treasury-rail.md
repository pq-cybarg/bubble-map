...
## Rollover, the de-rate break, and the duration assumption *(added 2026-06-16, #56)*
The rail doesn't just *manufacture* demand — it **changes the term structure of how the deficit is funded.** *Mechanics fact; break scenarios labeled.*

- **Rollover mechanics.** Reserves sit at the **shortest tenor** (cash, repo, ≤3-month bills), so the whole stack rolls **~4×/yr or more**. As the Treasury front-loads *bill* issuance to meet the bid, the **weighted-average maturity of the federal debt shortens** — a larger share of $37.6T must be refinanced each year at the prevailing rate. **Cheaper coupons now, more frequent rate-resets later** — the rail trades duration for current cost.
- **The de-rate break.** The captive bid is procyclical and rate-sensitive, so it can weaken exactly when the Treasury is most dependent: **(1) Fed cuts** compress bill yields → erode issuer economics and the appeal of holding non-yield stablecoins → growth stalls while bills still must roll; **(2) crypto risk-off / depeg** → forced fast bill sales (the contagion edge); **(3) rollover into thin demand** → redemptions and a roll date coincide and the issuer is a forced seller into its own roll.
- **The duration assumption.** The load-bearing assumption is that stablecoin AUM is **sticky, permanent funding** — but a stablecoin is **redeemable-on-demand (overnight-callable) money**. That's maturity transformation at **sovereign scale**: instantly-redeemable claims funding the government's front-end rollover. **"Stablecoin AUM is stable" is the self-marked number here** — the same defect as bank HTM marks, AI fair-value, private-credit NAV, and GPU useful-life ([[self_marked_value]]): a value held above its run-exposed realizable level until a forcing event. It is the **SVB mismatch relocated to the sovereign funding base** — the "deposit" is the token, the "asset" is the bill, and the marginal Treasury holder now holds callable money.
...
## The risks — *contested*
- **Run risk:** a lost peg forces a fast **T-bill fire-sale** — a stablecoin run becomes a Treasury shock (the `Blockchain_Stablecoin → Treasuries` contagion edge; BIS/ECB/S&P flag it; proponents say bills are liquid).
- **Opacity/concentration:** Tether reports **attestations, not full audits**; >$100B of US debt in one offshore entity.
- **Conflict:** **USD1** (Trump-family **World Liberty Financial**) is itself a GENIUS-Act stablecoin — the administration writing the demand rule has a family stake in an issuer (`spec-exchanges-asia`: MGX's $2B into Binance settled in USD1). *The rule-writer is also a beneficiary.*
