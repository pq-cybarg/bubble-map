...
LABELS={  # explicit overrides (sinks, SPVs, compounds)
 ...
 "Meta_AI_ChildSafety":"Meta AI chatbots — child-safety nexus","Outlier":"Outlier (Scale AI labor platform)","Alignerr":"Alignerr (AI-training labor)","Character_AI":"Character.AI",
 "Singham":"Neville Roy Singham","Singham_Network":"Singham-funded nonprofit network (People's Forum/CodePink/PSL overlap)","Bitcoin_Policy_Institute":"Bitcoin Policy Institute (report publisher)","AI_Datacenter_Foreign_Influence_Claim":"'Foreign influence vs American AI' thesis (BPI series)",
 "Planet_Labs":"Planet Labs (PL)","Vantor":"Vantor (ex-Maxar Intelligence)","BlackSky":"BlackSky (BKSY)","Satellogic":"Satellogic (SATL)","Capella_Space":"Capella Space (SAR, IonQ-owned)","Umbra":"Umbra (SAR)","ICEYE":"ICEYE (SAR, Finland)","Spire_Global":"Spire Global (SPIR)","HawkEye360":"HawkEye 360 (RF, HAWK)","Albedo":"Albedo (VLEO, pivoted)","Airbus_DS":"Airbus Defence & Space","Synspective":"Synspective (SAR, Japan)","ChangGuang":"ChangGuang / Jilin-1 (China, sanctioned)","Spacety":"Spacety (China, sanctioned)","NGA":"NGA (geospatial-intel agency)","SDA":"Space Development Agency","Rocket_Lab":"Rocket Lab (RKLB)","ULA":"United Launch Alliance","Blue_Origin":"Blue Origin","Firefly":"Firefly Aerospace (FLY)","Sierra_Space":"Sierra Space","Advent_International":"Advent International (owns Vantor)","Commercial_Space_Imaging":"Commercial space-imaging / EO layer",
 "American_Edge_Project":"American Edge Project (Meta dark-money 501c4)","Meta_State_SuperPACs":"Meta state super-PACs (METAC/ATEP)","Digital_Childhood_Alliance":"Digital Childhood Alliance (app-store front group)","National_PTA":"National PTA (ex-Meta sponsor)","Targeted_Victory":"Targeted Victory (GOP PR — anti-TikTok op)","NetChoice":"NetChoice (tech-industry litigation arm)","Louisiana_HB570":"Louisiana HB-570 (app-store age-verification law)",
 "Julie_Inman_Grant":"Julie Inman Grant (Australia eSafety Commissioner)",
 "Dialog_Society":"Dialog (Thiel/Hoffman private elite forum)","Stonebrick_LLC":"Stonebrick LLC (Hoffman — operates Dialog)","Peter_Thiel":"Peter Thiel (Dialog co-founder)","Auren_Hoffman":"Auren Hoffman (Dialog co-founder/chairman)","Raffi_Grinberg":"Raffi Grinberg (Dialog executive director)","Thiel_Foundation":"The Thiel Foundation","Founders_Fund":"Founders Fund","Andreessen_Horowitz":"Andreessen Horowitz (a16z)",
 "Arbitration_ADR_System":"Arbitration / ADR system (private dispute resolution)","American_Arbitration_Association":"American Arbitration Association (AAA/ICDR)","JAMS":"JAMS (ADR)","FINRA_DR":"FINRA Dispute Resolution (securities arb)","FINRA":"FINRA (securities SRO)","National_Arbitration_Forum":"National Arbitration Forum (2009 scandal)","FMCS":"FMCS (federal labor mediation)","National_Mediation_Board":"National Mediation Board (rail/air labor)","Federal_Arbitration_Act":"Federal Arbitration Act (1925)","Mass_Arbitration":"Mass arbitration (plaintiff counter-move)","Keller_Postman":"Keller Postman (mass-arb firm)","Institute_for_Legal_Reform":"US Chamber Institute for Legal Reform","CFPB":"CFPB (arbitration rule, repealed 2017)","ICC_Arbitration":"ICC Court of Arbitration (Paris)","LCIA":"LCIA (London)","SIAC":"SIAC (Singapore)","HKIAC":"HKIAC (Hong Kong)","CIETAC":"CIETAC (China)","New_York_Convention":"New York Convention (1958)","UNCITRAL":"UNCITRAL (Model Law + WG III)","ISDS":"Investor-State Dispute Settlement","ICSID":"ICSID (World Bank ISDS forum)","Energy_Charter_Treaty":"Energy Charter Treaty (ISDS; EU exodus)","PCA_Arbitration":"Permanent Court of Arbitration (The Hague)","CAS":"Court of Arbitration for Sport (Lausanne)","State_ADR":"State court-annexed / agency ADR","Beth_Din":"Beth Din / religious arbitration","Kleros":"Kleros (on-chain juror protocol)","Aragon_Court":"Aragon Court (DAO oracle, defunct)","UMA_Protocol":"UMA Optimistic Oracle (Polymarket resolver)",
 ...
 "Auto_Credit_Unwind":"Auto-credit unwind (2025)","Carvana":"Carvana","DriveTime":"DriveTime (Garcia)","Ernest_Garcia":"Ernest Garcia II","Fifth_Third":"Fifth Third","Tricolor_Holdings":"Tricolor (subprime auto)","First_Brands":"First Brands","Subprime_Auto_ABS":"Subprime auto ABS",
 ...
 # asbestos
 "Asbestos_Trusts":"Asbestos §524(g) trusts","Johns_Manville":"Johns-Manville","Mesothelioma_Claims":"Mesothelioma claims","EPA_Asbestos_Ban":"EPA chrysotile ban","Asbestos_Trust_Transparency":"Asbestos trust-transparency debate",
...
#flk{display:none;margin:6px 0 2px;background:var(--paper);border:1px solid var(--line);border-radius:8px;padding:9px 11px;max-width:340px}
#flk.on{display:block}
#flk .row{display:flex;align-items:center;gap:8px;margin:4px 0;font:11px/1.2 -apple-system,Segoe UI,Roboto,sans-serif}
#flk .row label{flex:0 0 118px;color:var(--mut)}
#flk .row input[type=range]{flex:1;min-width:0}
#flk .row .v{flex:0 0 40px;text-align:right;font-variant-numeric:tabular-nums;color:#1c1b19}
#flk .btns{display:flex;gap:8px;margin-top:9px}
#flk button{cursor:pointer;font:11px -apple-system,Segoe UI,Roboto,sans-serif;border:1px solid var(--line);border-radius:6px;padding:5px 10px;background:#fff;color:var(--ac2)}
#flk button:hover{background:var(--panel-bg,#f3eedf)}
...
const link=root.append('g').selectAll('path').data(LINKS).join('path').attr('fill','none')
 .attr('stroke',d=>d.layer==='financial'?(d.circular?'#c0392b':'#8f836a'):'#b3a789')
 .attr('stroke-width',d=>d.circular?1.9:(d.layer==='financial'?1.2:0.9))
 .attr('stroke-dasharray',d=>d.layer==='structural'?'3,3':null).attr('opacity',.8)
...
let clustered=true;
// live-tunable flocking parameters (defaults = the baked-in layout); the on-map panel edits these
const FLK={velDecay:.55,alphaDecay:.035,charge:-70,chargeMax:240,linkStr:.12,linkDist:46,
           cohesion:.6,collidePad:11,collideStr:.9,collideIter:2,spreadX:.46,spreadY:.5};
...
// subsidiaries need a MUCH stronger pull to their parent than the weak global sector cohesion,
// or a child with links elsewhere freezes stranded far away. childPull overrides FLK.cohesion for
// nodes that have a live parent (in aggregate mode); everyone else keeps the normal cohesion.
const cohStr=d=>(aggregate&&d.parent&&id2n.has(d.parent))?Math.max(FLK.cohesion,0.92):(clustered?FLK.cohesion:.05);
...
// DETERMINISTIC PER-BUCKET SEED: seed each sector as a COMPACT disk around its own anchor, BEFORE the
// sim runs. Two reasons: (1) d3 otherwise seeds all nodes on one phyllotaxis at the origin, and the
// settle-freeze (alpha<0.06, ~80 ticks) fires before weakly-forced leaf nodes (606 deg-1 leaves) can
// reach their cluster - so they froze piled at the centre; (2) seeding with a GLOBAL index flung
// high-index nodes far from their anchor, so they started already overlapping neighbouring sectors
// ("bubbles bleed into other sectors"). Indexing WITHIN each bucket keeps every sector a tight disk,
// sized by its own membership, that starts well clear of its neighbours.
(function(){const GA=2.399963229;   // golden angle -> even, deterministic spread (no Math.random)
 const byB={}; NODES.forEach(n=>{(byB[n.bucket]=byB[n.bucket]||[]).push(n);});
 Object.keys(byB).forEach(b=>{const arr=byB[b],ax=bAnchor(b,'x'),ay=bAnchor(b,'y');
   arr.sort((p,q)=>(p.parent?1:0)-(q.parent?1:0));   // parents first so children can hug them
   arr.forEach((n,j)=>{
     if(n.parent){const p=id2n.get(n.parent); if(p&&p.x!=null){const a=j*GA;n.x=p.x+Math.cos(a)*22;n.y=p.y+Math.sin(a)*22;return;}}
     const a=j*GA, r=10+Math.sqrt(j)*6; n.x=ax+Math.cos(a)*r; n.y=ay+Math.sin(a)*r;});   // compact disk, radius grows with sqrt(count)
 });
})();
const sim=d3.forceSimulation(NODES)
 ...
 .force('charge',d3.forceManyBody().strength(FLK.charge).distanceMax(FLK.chargeMax))
 .force('x',d3.forceX(ancX).strength(cohStr)).force('y',d3.forceY(ancY).strength(cohStr))
 .force('collide',d3.forceCollide().radius(d=>rad(d)+FLK.collidePad+labPad(d)).strength(FLK.collideStr).iterations(FLK.collideIter))
 .on('tick',tick);
drawSectors();
// re-apply the live FLK params to the running forces (used by the tuning panel)
function applyFlk(reheat){
 sim.velocityDecay(FLK.velDecay).alphaDecay(FLK.alphaDecay);
 sim.force('link').strength(FLK.linkStr).distance(d=>d.layer==='financial'?FLK.linkDist:FLK.linkDist+18);
 sim.force('charge').strength(FLK.charge).distanceMax(FLK.chargeMax);
 sim.force('x').strength(cohStr); sim.force('y').strength(cohStr);
 sim.force('collide').radius(d=>rad(d)+FLK.collidePad+labPad(d)).strength(FLK.collideStr).iterations(FLK.collideIter);
 drawSectors(); if(reheat!==false){fitted=false; sim.alpha(.6).restart();}
}
function tick(){
...
function egoFocus(d){egoOn=d.id;const keep=ADJ.get(d.id)||new Set([d.id]);
 const S=(curK<1?Math.min(1/curK,3.2):1);
 node.style('opacity',n=>keep.has(n.id)?1:.08);
 labels.style('display',n=>keep.has(n.id)?null:'none');
 const direct=l=>lerp(l.source)===d.id||lerp(l.target)===d.id;   // edges touching the selected bubble
 const vis=l=>keep.has(lerp(l.source))&&keep.has(lerp(l.target));
 link.style('opacity',l=>direct(l)?1:(vis(l)?.4:.1))               // enhance the closer (direct) connections most
     .attr('stroke-width',l=>(direct(l)?baseW(l)*2.2:baseW(l))*S);
...
function clearEgo(){egoOn=null;const S=(curK<1?Math.min(1/curK,3.2):1);
 node.style('opacity',1);link.style('opacity',.8).attr('stroke-width',d=>baseW(d)*S);linkHit.style('pointer-events',null);
...
// ---- live flocking tuner: sliders bound to FLK, reheat on change, copy/reset ----
(function(){
 const box=document.getElementById('flk'); if(!box||typeof FLK==='undefined') return;
 const spec=[
  ['velDecay','friction',0.10,0.90,0.01],
  ['alphaDecay','cool-down',0.005,0.100,0.005],
  ['charge','repulsion',-300,-5,5],
  ['chargeMax','repulsion range',80,800,10],
  ['linkStr','link strength',0,0.60,0.01],
  ['linkDist','link distance',20,140,2],
  ['cohesion','sector cohesion',0,0.80,0.01],
  ['collidePad','collide padding',0,40,1],
  ['collideStr','collide strength',0.10,1.00,0.05],
  ['collideIter','collide iters',1,4,1],
  ['spreadX','sector spread X',0.10,0.60,0.01],
  ['spreadY','sector spread Y',0.10,0.60,0.01]
 ];
 const DEF=Object.assign({},FLK); let deb=null;
 spec.forEach(function(s){
  const k=s[0], row=document.createElement('div'); row.className='row';
  row.innerHTML='<label>'+s[1]+'</label><input type=range min='+s[2]+' max='+s[3]+' step='+s[4]+' value='+FLK[k]+'><span class=v>'+FLK[k]+'</span>';
  const rng=row.querySelector('input'), v=row.querySelector('.v');
  rng.addEventListener('input',function(){FLK[k]=parseFloat(rng.value); v.textContent=rng.value;
    clearTimeout(deb); deb=setTimeout(function(){applyFlk(true);},150);});
  box.appendChild(row);
 });
 const btns=document.createElement('div'); btns.className='btns';
 btns.innerHTML='<button id=flkReset>Reset</button><button id=flkCopy>Copy params</button>';
 box.appendChild(btns);
 document.getElementById('tFlk').addEventListener('change',function(e){box.classList.toggle('on',e.target.checked);});
 document.getElementById('flkReset').addEventListener('click',function(){
  Object.assign(FLK,DEF);
  [].forEach.call(box.querySelectorAll('.row'),function(row,i){const k=spec[i][0],r=row.querySelector('input');r.value=FLK[k];row.querySelector('.v').textContent=FLK[k];});
  applyFlk(true);});
 document.getElementById('flkCopy').addEventListener('click',function(){
  const txt=JSON.stringify(FLK); const b=document.getElementById('flkCopy');
  if(navigator.clipboard){navigator.clipboard.writeText(txt).then(function(){b.textContent='Copied!';setTimeout(function(){b.textContent='Copy params';},1200);});}
  else{prompt('FLK params:',txt);}
 });
})();
</script></body></html>"""
