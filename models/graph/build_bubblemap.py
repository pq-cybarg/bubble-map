...
.sect text{font:800 30px Georgia,serif;fill:#7b2d2610;letter-spacing:.12em;pointer-events:none;text-transform:uppercase}
...
const sectorG=root.insert('g',':first-child').attr('class','sect');   // faint sector headers, behind graph, pan/zoom with it
function drawSectors(){sectorG.selectAll('text').data(clustered?BUCKETS:[],b=>b).join('text')
 .attr('x',b=>bAnchor(b,'x')).attr('y',b=>bAnchor(b,'y')).attr('text-anchor','middle')
...
const sim=d3.forceSimulation(NODES)
 ...
 .force('x',d3.forceX(ancX).strength(FLK.cohesion)).force('y',d3.forceY(ancY).strength(FLK.cohesion))
...
function applyFlk(reheat){
 ...
 sim.force('x').strength(clustered?FLK.cohesion:.05); sim.force('y').strength(clustered?FLK.cohesion:.05);
...
function egoFocus(d){egoOn=d.id;const keep=ADJ.get(d.id)||new Set([d.id]);
 ...
 link.style('opacity',l=>vis(l)?.9:.16);
...
function clearEgo(){egoOn=null;
 node.style('opacity',1);link.style('opacity',.8);linkHit.style('pointer-events',null);
