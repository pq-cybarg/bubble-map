...
const sim=d3.forceSimulation(NODES)
 .velocityDecay(.55).alphaDecay(.035)              // more friction + faster cool-down => settles, no lingering jitter
 .force('link',d3.forceLink(LINKS).id(d=>d.id).distance(d=>d.layer==='financial'?46:64).strength(.12))  // weaker links so sectors hold
 .force('charge',d3.forceManyBody().strength(-90).distanceMax(340))                                       // local repulsion only
 .force('x',d3.forceX(ancX).strength(.34)).force('y',d3.forceY(ancY).strength(.36))                       // strong sector cohesion
 .force('collide',d3.forceCollide().radius(d=>rad(d)+11+labPad(d)).strength(.9).iterations(2))            // cleaner overlap resolution
 .on('tick',tick);
drawSectors();
function tick(){
...
document.getElementById('tCluster').onchange=e=>{clustered=e.target.checked;
 sim.force('x').strength(clustered?.34:.05);sim.force('y').strength(clustered?.36:.05);   // free layout = weak center only
