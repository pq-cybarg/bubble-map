...
function render3money(id, series, ztitle){{
  ...
  // ---- fallback (no WebGL): three clean side-by-side panels, one per money, shared log y ----
  const dom=[[0.00,0.30],[0.36,0.64],[0.70,1.00]], axx=["x","x2","x3"];
  const traces=[];
  series.forEach(function(s){{
    [0,1,2].forEach(function(p){{
      const xs=[],ys=[];
      s.pts.forEach(function(r){{ if(r.p===p&&r.v!=null){{xs.push(r.yr);ys.push(r.v);}} }});
      traces.push({{type:"scatter",mode:"lines+markers",name:s.name,legendgroup:s.name,showlegend:p===0,
        x:xs,y:ys,xaxis:axx[p],yaxis:"y",line:{{color:s.color,width:2}},marker:{{size:4,color:s.color}},
        visible:s.visible===false?"legendonly":true,
        hovertemplate:s.name+"<br>%{{x}} · "+PLANES[p]+"<br>index %{{y:.0f}}<extra></extra>"}});
    }});
  }});
  const layout={{margin:{{l:46,r:10,t:22,b:34}},paper_bgcolor:"#fcfcfb",plot_bgcolor:"#fcfcfb",
    legend:{{font:{{family:"-apple-system,Segoe UI,Roboto,sans-serif",size:12}},orientation:"h",y:-0.18}},
    yaxis:{{type:"log",title:ztitle,gridcolor:grid_c,color:ink,domain:[0,1]}},
    annotations:[]}};
  [0,1,2].forEach(function(p){{
    layout[p===0?"xaxis":("xaxis"+(p+1))]={{domain:dom[p],anchor:"y",gridcolor:grid_c,color:ink,tickformat:"d"}};
    layout.annotations.push({{text:PLANES[p],x:(dom[p][0]+dom[p][1])/2,y:1.0,xref:"paper",yref:"paper",
      yanchor:"bottom",yshift:2,showarrow:false,font:{{color:"#7b2d26",size:12}}}});
  }});
