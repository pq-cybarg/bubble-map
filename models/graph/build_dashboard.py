...
OVERLAYS=[
 ...
 ("The operator-network (not one cabal)","A small recurring incentive-aligned network - Thiel/PayPal & a16z + Altman + Musk + the Gulf funds - now holding founder, funder AND official positions across the AI core, defense-tech (Palantir Pentagon backbone, Anduril), crypto (Sacks AI/crypto czar, Bitcoin Reserve), digital-ID (World/Persona) and the executive branch (Vance VP, Musk/DOGE). Intent not asserted; the people who built the layers increasingly write the rules","influence-operator-network"),
]
